...
if rp['source'] is 'mcps':
    ...
    rp['data_fnamelist'] = ['bessell_{0}'.format(f) for f in ['U','B','V','R']]
...
#you can add parameters here by giving a dict describing
#their distribution - don't worry, this distribution can be changed later
fitter.params['UV_SLOPE'] = {'min':0.5, 'max':2.0, 'type':'linear'}
fitter.initialize_grid(params = None)
fitter.set_params_from_isochrone(Z = [0.0077], logt_min = rp['logt_min'],
                                 logl_min = rp['logl_min'], logt_max = 4.65) 
#set extra dust parameters (should marginalize over, but requires thought about prior)
fitter.stargrid.pars['R_V'] = 3.0 
fitter.stargrid.pars['F_BUMP'] = 1.0 #2.*np.random.beta(2, 2, rp['ngrid'])
fitter.stargrid.pars['UV_SLOPE'] = 1.0
#fitter.stargrid.pars.add_par(fitter.stargrid.pars['R_V']*fitter.stargrid.pars['A_V'], ['E(B-V)'])
